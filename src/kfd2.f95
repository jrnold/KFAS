subroutine kfd2(yt, ymiss, timevar, zt, ht,tt, rt, qt, a1, p1, p1inf, vt, ft,kt,&
finf, kinf, d, j, p, m, r, n, lik, tolf,rankp)
    ...
    double precision, intent(inout), dimension(p,n) :: vt,ft
    double precision, intent(inout), dimension(m,p,n) :: kt,kinf
    double precision, intent(inout),dimension(p,n) ::  finf
    double precision, intent(inout) :: lik
    double precision, dimension(m) :: at,arec
    double precision, dimension(m,m) :: prec,pirec,im,mm,pinf,pt
    ...
    j=0
    d=0
    if(maxval(pinf(1:m,1:m)) .GT.  0.0d0) then
        ...
        if(d.EQ.0) then
            ...
            at(1:m) = a1 !call dcopy(m,a1,1,at(1:m,1),1) !at(:,1) = a1
            pt(1:m,1:m) = p1
        end if
        do t = d+1, n
           do i = 1, p
              if(ymiss(t,i).EQ.0) then
                 ...
                 !lik = lik - 0.5*(log(ft(i,t)) + vt(i,t)**2/ft(i,t))
              end if
           end do
            ...
            !pt(1:m,1:m) = pt(1:m,1:m) + qt(1:r,1:r,(t-1)*timevar(5)+1)
